...
def fake_app(monkeypatch):
    ...
    fake_app.account_handler = types.SimpleNamespace(
        get_resy_token=lambda uid: 'token',
        remove_resy_token=lambda uid: None
    )
    ...
    forms_pkg = types.ModuleType('app.forms'); forms_pkg.__path__ = []
    rf = types.ModuleType('app.forms.resy_form');
    class ResyTokenForm: pass
    ...
    routes_pkg = types.ModuleType('app.routes'); routes_pkg.__path__ = ['backend/app/routes']
...
def create_app(module_path):
    ...
    app = Flask(__name__)
    app.register_blueprint(mod.__getattribute__(module_path.split('.')[-1] + '_bp'))
    return app
